...
async function requireAdmin() {
  ...
  if (!user) return null;
  const { data: profile } = await supabase.from("profiles").select("is_staff, is_admin").eq("id", user.id).single();
...
Клуб принимает собак до 15 кг и кошек без ограничений по весу.
