# Self-hosted Supabase для Лапа Клуб (РФ)

> Фактическое размещение: приложение — Amvera (контейнер в ЦОД Yandex Cloud, РФ),
> БД `db.lapaclub.ru` — сервер Timeweb Cloud (93.183.82.170, netname TW-Cloud, РФ).
> Ниже по тексту «Amvera» относится к приложению.
