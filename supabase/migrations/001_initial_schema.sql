...
insert into public.reviews (author_name, text, rating, pet_type, is_published) values
  ('Анна К.', 'Оставляла кошку на неделю — всё прошло отлично! Команда очень внимательная, регулярно присылали фото. Вернёмся обязательно.', 5, 'кошка', true),
  ('Михаил Д.', 'Отличное место для нашего пса. Доброжелательный персонал, чистые боксы. Рекомендую всем в Калуге!', 5, 'собака', true),
  ('Светлана В.', 'Водим дочкину собаку в детский сад на полный день. Очень удобно — забираем уставшую и счастливую.', 5, 'собака', true);
